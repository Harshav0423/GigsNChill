...
export default function EventPage(props) {
  ...
  // useEffect(() => {
  //   const u_info = JSON.parse(localStorage.getItem("user"));
  //   if (u_info && u_info.isLoggedIn) {
  //     console.log("In here at EvPage");
  //     navigate("/login");
  //   }
  // }, []);
  ...
  useEffect(() => {
    ...
    axios(config).then((res) => {
      ...
      axios(config2).then((res) => {
        ...
        try {
          let bookedevent = futureEvents.some(
            (item) => item.EventName === eventId.EventName
          );
      ...
      axios(config3).then((res) => {
        ...
        try {
          let eventBookmarked = bookmarks.some(
            (item) => item.EventId === eventId.EventId
          );
  ...
  async function countPlus() {
    await setNoftickets((prev) =>
      prev + 1 <= event.SeatsAvailable ? prev + 1 : prev
    );
          ...
          <Card className={styles.container}>
            ...
            <div className={styles.cardcontent}>
              <CardContent>
                ...
                <div>
                  <p>Price of Each Ticket : {event.Price}</p>
                </div>
              </CardContent>
              ...
              <div className={styles.pricediv}>
                <p>Total Price : {noftickets * event.Price}</p>
              </div>
              <div className={styles.eventregister}>
                <button
                  type="button"
                  disabled={isBooked === true ? true : false}
